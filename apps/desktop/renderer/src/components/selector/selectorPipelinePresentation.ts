...
const DEFAULT_MODE_COPY: Record<SelectorPipelineMode, { label: string; description: string }> = {
  legacy: {
    label: "Legacy",
...
function translatedLabel(t: TFunction | undefined, key: string, fallback: string, options?: Record<string, unknown>) {
  return t ? t(key, options) : fallback;
}

export function getSelectorPipelineLabel(diagnostics: SelectorPipelinePresentationInput, t?: TFunction) {
  const isFallback = diagnostics.executionStatus === "fallback" || Boolean(diagnostics.fallback) || diagnostics.status === "fallback";
  let baseLabel: string;

  if (isFallback) {
    baseLabel = translatedLabel(t, "selectorDiagnostics.badges.legacyFallback", "Legacy fallback");
  } else if (diagnostics.requestedMode === "shadow_compare") {
    baseLabel = translatedLabel(t, "selectorDiagnostics.badges.compareLegacyOutput", "Compare · Legacy output");
  } else if (diagnostics.effectivePipeline === "shadow") {
    baseLabel = translatedLabel(t, "selectorDiagnostics.badges.shadow", "Shadow");
  } else {
    baseLabel = translatedLabel(t, "selectorDiagnostics.badges.legacy", "Legacy");
  }
