...
function getBrandColor(
  ...
  contrast: AiToolLogoProps["contrast"] = "default",
) {
  const normalized = normalizeTool(tool);
  ...
  const hex = String(icon?.hex || "")
    .replace("#", "")
    .toLowerCase();

  if (!hex || hex === "000000" || hex === "111111") {
    return contrast === "onLight" ? "#18181b" : "#f5f5f5";
  }
  ...
  return (
    <span
      ...
      style={{
        ...
        backgroundColor:
          contrast === "onLight" ? "rgba(24, 24, 27, 0.06)" : `${color}14`,
      }}
      title={icon.title}
    >
      <svg
