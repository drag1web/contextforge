...
function expandRouteTaskTokens(tokens: string[]) {
    const expanded = new Set(tokens);

    for (const token of tokens) {
        const normalized = normalizeForCompare(token);

        // Universal cross-language route vocabulary. These are not project-specific
        // shortcuts; they let natural user wording such as "роут прайса" or
        // "раздел тарифов" match real inventory evidence like pricing/plans.
        if (normalized.startsWith("прайс") || normalized === "price" || normalized === "prices") {
            ["price", "prices", "pricing", "plan", "plans", "tier", "tiers"].forEach((item) => expanded.add(item));
        }

        if (normalized.startsWith("тариф") || normalized === "tariff" || normalized === "tariffs") {
            ["tariff", "tariffs", "price", "pricing", "plan", "plans", "tier", "tiers"].forEach((item) => expanded.add(item));
        }

        if (normalized.startsWith("подпис") || normalized === "subscription" || normalized === "subscriptions") {
            ["subscription", "subscriptions", "billing", "payment", "payments", "pricing", "plan", "plans"].forEach((item) => expanded.add(item));
        }

        if (normalized.startsWith("оплат") || normalized === "payment" || normalized === "payments") {
            ["payment", "payments", "billing", "checkout", "invoice", "invoices"].forEach((item) => expanded.add(item));
        }
    }
...
function isAmbiguousNaturalRouteMatch(rows: Array<{ candidate: InventoryRouteCandidate; score: number }>, explicitRoutes: string[]) {
    if (explicitRoutes.length > 0 || rows.length < 2) return false;
    const [first, second] = rows;
    if (!first || !second) return false;
    if (first.score < 48 || second.score < 48) return false;
...
function extractNaturalRouteMentions(input: SelectTaskFilesInput, explicitRoutes: string[]) {
    ...
    const taskTokens = expandRouteTaskTokens(uniqueStrings(tokenize([
        positiveText,
        ...(input.taskIntent?.domainTerms ?? []),
        ...(input.taskIntent?.mentionedEntities ?? []),
        ...(input.taskIntent?.recommendedSearchTerms ?? [])
    ...
    const rows = getInventoryRouteCandidates(input.inventory)
    ...
    if (isAmbiguousNaturalRouteMatch(rows, explicitRoutes)) return [];
...
function getExistingRouteTargetSeedFiles(input: SelectTaskFilesInput, area: EffectiveTaskArea, assetMode: AssetMode, tokenContext: TokenContext, selected: SelectedTaskFile[]) {
    if (tokenContext.routeMentions.length === 0) return [];

    const seen = new Set(selected.map((file) => normalizeForCompare(file.path)));
    const seeds: SelectedTaskFile[] = [];
    const nonRootRoutes = tokenContext.routeMentions
        .map(normalizeRouteValue)
        .filter((route) => route && route !== "/");

    for (const route of nonRootRoutes) {
        const routeMatches = input.inventory.files
            .filter((file) => !seen.has(normalizeForCompare(file.path)))
            .filter((file) => isPageLikeTargetFile(file))
            .filter((file) => canUseSelectedFile(input, file, area, assetMode))
            .map((file) => {
                const routePath = file.routePath ? normalizeRouteValue(file.routePath) : "";
                const exactRoute = Boolean(routePath && routePath === route);
                const directRoute = isDirectRoutePageMatch(file, [route]);
                const routeScore = getRouteMatchScore(file, [route]);
                return {
                    file,
                    score: (exactRoute ? 1000 : 0) + (directRoute ? 500 : 0) + routeScore + getPageSemanticMatchScore(file, input, tokenContext) * 0.25
                };
            })
        ...
        seen.add(normalizeForCompare(best.file.path));
        seeds.push(makeSelectedFile(
            best.file,
            `Selected as the existing route/page target for ${route}; this real inventory file takes priority over landing/reference pages.`,
            0.96,
            "inspect-and-edit"
        ));
    }

    return seeds.slice(0, getConcretePageTargetLimit(input, area, tokenContext));
}
...
function buildFallbackSelection(input: SelectTaskFilesInput): TaskFileSelection {
    ...
    for (const routeTargetFile of getExistingRouteTargetSeedFiles(input, effectiveTaskArea, assetMode, tokenContext, selected)) {
        selected.push(routeTargetFile);
    }
